% pop_encoding_slope_JCSOinTT_redoslope.m
%
...
%    between JC and SO
% 4. tuning curve difference between SO offer1 and SO offer2 and order bias
%    of SO
% 
...
% revision:   Nov 2020: WS - redo the fitting, give the option to use OVA(B) instead of
%                            OVA(B)|AB(BA) etc 
...
if      juiceABpooled &  slopepooled
    figuresave = ['C:\Experiments\TwoTasks\Analysis\Analysis_OFC\pop_encoding_slope_JCSOinTT_redoslope_juiceABpooled_slopepooled_',monkey_ana];
elseif  juiceABpooled & ~slopepooled
    figuresave = ['C:\Experiments\TwoTasks\Analysis\Analysis_OFC\pop_encoding_slope_JCSOinTT_redoslope_juiceABpooled_',monkey_ana];
elseif ~juiceABpooled &  slopepooled
    figuresave = ['C:\Experiments\TwoTasks\Analysis\Analysis_OFC\pop_encoding_slope_JCSOinTT_redoslope_slopepooled_',monkey_ana];
elseif ~juiceABpooled & ~slopepooled
    figuresave = ['C:\Experiments\TwoTasks\Analysis\Analysis_OFC\pop_encoding_slope_JCSOinTT_redoslope_',monkey_ana];    
end
...
chhystornot = '_chhyst'; % "", "_chhyst", "_chhyst_singlefit", '_chhyst_ABBAsep'
probitorlogit = '_probit'; % '_probit', '_logit'
withNANornot = ''; % '_withNAN', ''
...
dosteepout = 1;     % 1 if apply the steepness outlier detection
% Gervinho
if removesessions
    filename = ['pop_behav_ana_summary_Gervinho_removesessions'];
elseif ~removesessions
    filename = ['pop_behav_ana_summary_Gervinho'];
end
eval(['load ', filename])
...
% Juan
if removesessions
    filename = ['pop_behav_ana_summary_Juan_removesessions'];
elseif ~removesessions
    filename = ['pop_behav_ana_summary_Juan'];
end
eval(['load ', filename])
quantiles_JC = quantile(steepness_all(:,1),[0.25 0.5 0.75]);
IQR_JC = quantiles_JC(3) - quantiles_JC(1);
...
for iclassname = 1:nclassnames
    ...
    for islopesign = 1:nslopesigns
        ...
        try            
            filename = ['pop_encoding_slope_JCSOinTT_redoslope',differentRho,'_',monkey_ana,probitorlogit,chhystornot,withNANornot];
%             filename = ['pop_encoding_slope_JCSOinTT_redoslope',differentRho,'_',monkey_ana,'_logit'];
            load(filename);     
            %
%              exnovo
            %
            ...
            for icell = 1:ncells           
                ...
                try
                    ...
                    if isequal(chhystornot, '')
                        ...
                        orderbias_inSO(icell,:) = 2*relvalue_SO*psyphycell.SO.NonChHyst.sigmoidfit.beta(3)./psyphycell.SO.NonChHyst.sigmoidfit.beta(2);  
                    ...
                    elseif isequal(chhystornot, '_chhyst_singlefit')
                        relvalue_SO = exp(-(psyphycell.SO.NoOrdBias.sigmoidfit.beta(1))/(psyphycell.SO.NoOrdBias.sigmoidfit.beta(2)));
                        relvalue_JC = exp(-(psyphycell.JC.OrdChHyst.sigmoidfit.beta(1))/(psyphycell.JC.OrdChHyst.sigmoidfit.beta(2)));
                        steepness_inSO(icell,:) = psyphycell.SO.NoOrdBias.sigmoidfit.beta(2);
                        steepness_inJC(icell,:) = psyphycell.JC.OrdChHyst.sigmoidfit.beta(2);
                        orderbias_inSO(icell,:) = 2*relvalue_SO*psyphycell.SO.NonChHyst.sigmoidfit.beta(3)./psyphycell.SO.NonChHyst.sigmoidfit.beta(2);  
                        chhyst_inSO(icell,:) = 2*relvalue_SO*psyphycell.SO.NoOrdBias.sigmoidfit.beta(3)./psyphycell.SO.NoOrdBias.sigmoidfit.beta(2);
                        chhyst_inJC(icell,:) = 2*relvalue_JC*psyphycell.JC.OrdChHyst.sigmoidfit.beta(3)./psyphycell.JC.OrdChHyst.sigmoidfit.beta(2);
                        if isempty(differentRho)
                            rho_inJC(icell,:) = (relvalue_SO + relvalue_JC)/2;
                            rho_inSO(icell,:) = (relvalue_SO + relvalue_JC)/2;
                    ...
                    elseif isequal(chhystornot, '_chhyst') 
                        relvalue_SO = exp(-(psyphycell.SO.OrdChHyst.sigmoidfit.beta(1))/(psyphycell.SO.OrdChHyst.sigmoidfit.beta(2)));
                        relvalue_JC = exp(-(psyphycell.JC.OrdChHyst.sigmoidfit.beta(1))/(psyphycell.JC.OrdChHyst.sigmoidfit.beta(2)));
                        steepness_inSO(icell,:) = psyphycell.SO.OrdChHyst.sigmoidfit.beta(2);
                        steepness_inJC(icell,:) = psyphycell.JC.OrdChHyst.sigmoidfit.beta(2);
                        orderbias_inSO(icell,:) = 2*relvalue_SO*psyphycell.SO.OrdChHyst.sigmoidfit.beta(3)./psyphycell.SO.OrdChHyst.sigmoidfit.beta(2);  
                        chhyst_inSO(icell,:) = 2*relvalue_SO*psyphycell.SO.OrdChHyst.sigmoidfit.beta(4)./psyphycell.SO.OrdChHyst.sigmoidfit.beta(2);
                        chhyst_inJC(icell,:) = 2*relvalue_JC*psyphycell.JC.OrdChHyst.sigmoidfit.beta(3)./psyphycell.JC.OrdChHyst.sigmoidfit.beta(2);
                        if isempty(differentRho)
                            rho_inJC(icell,:) = (relvalue_SO + relvalue_JC)/2;
                    ...
                    elseif isequal(chhystornot,'_chhyst_ABBAsep')
                        relvalue_SO = exp(-(psyphycell.SOBA.OrdChHyst.sigmoidfit.beta(1))/(psyphycell.SOBA.OrdChHyst.sigmoidfit.beta(2)));
                        relvalue_JC = exp(-(psyphycell.JC.OrdChHyst.sigmoidfit.beta(1))/(psyphycell.JC.OrdChHyst.sigmoidfit.beta(2)));
                        steepness_inSO(icell,:) = psyphycell.SOBA.OrdChHyst.sigmoidfit.beta(2);
                        steepness_inJC(icell,:) = psyphycell.JC.OrdChHyst.sigmoidfit.beta(2);
                        % orderbias_inSO(icell,:) = 2*relvalue_SO*psyphycell.SO.OrdChHyst.sigmoidfit.beta(3)./psyphycell.SO.OrdChHyst.sigmoidfit.beta(2);
                        orderbias_inSO(icell,:) = exp(-(psyphycell.SOAB.OrdChHyst.sigmoidfit.beta(1))/(psyphycell.SOAB.OrdChHyst.sigmoidfit.beta(2)))-exp(-(psyphycell.SOBA.OrdChHyst.sigmoidfit.beta(1))/(psyphycell.SOBA.OrdChHyst.sigmoidfit.beta(2)));
                        chhyst_inSO(icell,:) = 2*relvalue_SO*psyphycell.SOBA.OrdChHyst.sigmoidfit.beta(3)./psyphycell.SOBA.OrdChHyst.sigmoidfit.beta(2);
                        chhyst_inJC(icell,:) = 2*relvalue_JC*psyphycell.JC.OrdChHyst.sigmoidfit.beta(3)./psyphycell.JC.OrdChHyst.sigmoidfit.beta(2);
                        if isempty(differentRho)
                            rho_inJC(icell,:) = (relvalue_SO + relvalue_JC)/2;
                            rho_inSO(icell,:) = (relvalue_SO + relvalue_JC)/2;
                ...
                for iTW = 1:ntwins
                    ...
                    if ~isequal(withNANornot, '_withNAN')    
                        valAinBA = valA;
    %                     valAinBA(ABBA== 1) = nan;
                        valAinBA(ABBA== 1) = 0;
                        valAinAB = valA;
...
%                         CV = chV2;   % OV2 when 2 is chosen
                    ...
                    FR_inSO(icell,iTW) = nanmean(act_SO); 
                    ...
                    if isequal(classname,'CV')
                        ...
                        g = fittype('m*x+q','coeff',{'m','q'});
                        [linfit,goodness] = fit(XXX,YYY,g);
                        slopes_QAB_inSO(icell,iTW+2) = linfit.m;
...
filename = ['pop_encoding_slope_JCSOinTT_redoslope',differentRho,'_',monkey_ana,probitorlogit,chhystornot,withNANornot];
% eval(['save ',filename ' allchhyst allrhos allslopes allslopes_QAB allnonzeros allRsq allintcepts allmaxinters '...
%                        ' allFRs allSteepness allOrderbias allsessrange allcellnames allmonkeys '])
eval(['save ',filename ' allchhyst allrhos allslopes allnonzeros allRsq allintcepts allmaxinters '...
                       ' allFRs allSteepness allOrderbias allsessrange allcellnames allmonkeys '])
